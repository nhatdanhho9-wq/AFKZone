...
if [ -n "${ANTIGRAVITY_AGENT:-}" ]; then
	# Disable settings that could cause issues with the shell integration
	# We'd rather have the shell integration work than preserve user customizations

	# Affects whether we can successfully use preexec / precmd. Don't try to reuse
	# the user's bash preexec since it may have been set up incorrectly / too old.
	trap - DEBUG
	PROMPT_COMMAND=""
	unset $(compgen -v | grep '^__bp')
	unset bash_preexec_imported

	# extdebug has to be off for vsc_preexec to work properly
	shopt -u extdebug

	# Affects whether we can use `history 1` to get the last command accurately.
	unset HISTCONTROL

	# make sure lithist and cmdhist are disabled; newlines in there mess things up
	shopt -u lithist
	shopt -u cmdhist
fi
...
__vsc_preexec() {
	...
	if [[ ! $BASH_COMMAND == __vsc_prompt* ]]; then
		...
		if [ "$__vsc_history_verify" = "1" ]; then
			__vsc_current_command="$(HISTTIMEFORMAT= builtin history 1 | sed 's/ *[0-9]* *//')"
		else
