...
if [ -n "${ANTIGRAVITY_AGENT:-}" ]; then
	# Disable powerline10k *before* it gets loaded for agent terminals.  It seems
	# to interfere with shell integration in a pretty complicated way & can't
	# be disabled cleanly after loading.

	# Disable instant prompt
	typeset -g POWERLEVEL9K_INSTANT_PROMPT=off

	# Disable the actual powerline10k script by pretending it was sourced
	typeset -g __p9k_sourced=disabled_by_antigravity
fi
