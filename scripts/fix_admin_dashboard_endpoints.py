#!/usr/bin/env python3
"""Fix admin dashboard endpoints in app.py"""
...
# 1. Fix /admin/users endpoint - query from license_devices
if 'FROM devices WHERE' in content and '@app.get("/admin/users")' in content:
    # Replace the query section
    old_query = '''    # Get total count
    ...
    new_query = '''    # Adjust where clauses for license_devices
    ld_where_clauses = []
    if tier:
        ld_where_clauses.append("l.tier=:tier")
        params["tier"] = tier
    if search:
        ld_where_clauses.append("(ld.device_id ILIKE :search OR d.device_model ILIKE :search)")
        params["search"] = f"%{search}%"
    ld_where_sql = " AND ".join(ld_where_clauses) if ld_where_clauses else "1=1"
    ...
    query = f"""
        ...
        WHERE ld.is_active=TRUE AND ({ld_where_sql})
    ...
    if old_query in content:
        ...
        changes_made.append('Fixed /admin/users endpoint')
    
    # Also remove status filter
    content = content.replace(
        '    if status:\n        where_clauses.append("license_status=:status")\n        params["status"] = status',
        '    # Status filter removed (not applicable for license_devices)'
    )

# 2. Fix dashboard stats - expires_at comparison
if 'expires_at > EXTRACT(EPOCH FROM NOW()) * 1000' in content:
    content = content.replace(
    ...
    changes_made.append('Fixed dashboard stats SQL')

# 3. Add /admin/licenses/generate if missing
if '@app.post("/admin/licenses/generate")' not in content:
    # Find insertion point after bulk-create
...
'''
                ...
                changes_made.append('Added /admin/licenses/generate endpoint')

# 4. Add /admin/connections if missing
...
'''
                ...
                changes_made.append('Added /admin/connections endpoint')
