#!/usr/bin/env python3
"""
Apply account-based licensing endpoints to app.py
Adds user auth, user licenses, device management endpoints
"""
import re
...
def user_register(req: UserRegisterRequest, db: Session = Depends(get_db)):
    ...
    result = db.execute(text("""
        INSERT INTO users (email, password_hash, name, created_at) VALUES (:email, :hash, :name, NOW()) RETURNING id
...
def get_current_user(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    return {"user_id": user_data[0], "email": user_data[1], "name": user_data[2], "created_at": user_data[3].isoformat() if user_data[3] else None, "last_login": user_data[4].isoformat() if user_data[4] else None}
...
def get_user_licenses(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    for lic in licenses:
        ...
        result.append({"license_key": lic[0], "tier": lic[1], "duration_days": lic[2], "expires_at": expires.isoformat() if expires else None,
                       "status": status, "devices_max": lic[5] if lic[5] else 1, "devices_used": lic[6], "created_at": lic[7].isoformat() if lic[7] else None})
...
def get_user_activation_history(device_id: str, user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    for h in history:
        ...
        result.append({"license_key": h[0], "tier": h[1], "expires_at": expires.isoformat() if expires else None,
                       "activated_at": h[3].isoformat() if h[3] else None, "devices_max": h[4] if h[4] else 1, "devices_used": h[6], "status": status})
...
def get_user_devices(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    result = []
    for d in devices:
        result.append({"device_id": d[0], "alias": d[1], "last_seen": d[2].isoformat() if d[2] else None,
                       "activated_at": d[3].isoformat() if d[3] else None, "license_key": d[4], "tier": d[5]})
...
def clear_user_device(device_id: str, user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"success": True, "message": f"Device removed", "device_id": device_id}
...
if '@app.post("/auth/register")' in content:
    ...
    # Add before the last line (after all existing endpoints)
    # Find a good insertion point - before if __name__ == "__main__"
    if 'if __name__ ==' in content:
        content = content.replace('if __name__ ==', USER_AUTH_CODE + '\n\nif __name__ ==')
    else:
        content = content + USER_AUTH_CODE
    print("Added user auth and user endpoints")

# ==================== UPDATE PRODUCTS ENDPOINT ====================
# Replace the existing /products endpoint with one that includes color_hex
PRODUCTS_NEW = '''@app.get("/products")
...
# Find and replace existing products endpoint
old_products_pattern = r'@app\.get\("/products"\)\ndef get_products\(active_only: bool = True, db: Session = Depends\(get_db\)\):.*?return \{"products": products\}'
if re.search(old_products_pattern, content, re.DOTALL):
    print("Found old products endpoint, replacing...")
    content = re.sub(old_products_pattern, PRODUCTS_NEW, content, flags=re.DOTALL)
    print("Updated products endpoint with color_hex and tier sorting")
else:
    print("Products endpoint not found in expected format, manual update may be needed")
...
print("- GET /products (with color_hex)")
