"""
Account-Based Licensing - New API Endpoints
Add these endpoints to server_app.py

Includes:
- User Auth (register, login, me)
- User Licenses (list with slot info)
- Device Management (list, clear, alias)
- Products with color_hex and tier sorting
"""
...
def verify_user_token(authorization: str = Header(None)):
    """Verify user JWT token - use as Depends()"""
...
def user_login(req: UserLoginRequest, db: Session = Depends(get_db)):
    ...
    token = create_user_token(user[0], user[1])
...
def get_current_user(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    user_data = db.execute(text("""
        SELECT id, email, name, created_at, last_login FROM users WHERE id = :id
...
def get_user_licenses(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    """Get all licenses owned by user with slot info"""
    ...
    for lic in licenses:
        ...
        result.append({
            ...
            "status": status,
            "devices_max": lic[5] if lic[5] else 1,
            "devices_used": lic[6],
            "created_at": lic[7].isoformat() if lic[7] else None
        })
...
def get_user_activation_history(
    ...
    """Get activation history for a specific device (user's licenses only)"""
...
def get_user_devices(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    """Get all devices activated by user's licenses"""
    devices = db.execute(text("""
        SELECT DISTINCT ld.device_id, ld.alias, ld.last_seen, ld.activated_at, 
               ld.license_key, l.tier
        FROM license_devices ld
        JOIN licenses l ON ld.license_key = l.license_key
        WHERE l.user_id = :user_id
...
def clear_user_device(
    device_id: str,
    user: dict = Depends(verify_user_token),
    db: Session = Depends(get_db)
):
    """Clear/kick a device from user's license"""
    # Verify device belongs to user's license
    ...
    return {
        "success": True,
        "message": f"Device {device_id[:8]}... removed from license",
        "device_id": device_id
    }
...
def update_device_alias(
    ...
    db.execute(text("UPDATE license_devices SET alias = :alias WHERE device_id = :device_id"),
               {"alias": alias, "device_id": device_id})
    db.commit()
    
    return {"success": True, "device_id": device_id, "alias": alias}
'''

# ==================== PRODUCTS WITH COLOR_HEX ====================
PRODUCTS_ENDPOINT = '''
@app.get("/products")
def get_products(active_only: bool = True, db: Session = Depends(get_db)):
    ...
    for r in results:
        ...
        products.append({
            ...
            "duration_days": r[3],
            "price": price,
            "display_price": f"{price:,.0f}d".replace(",", "."),
            "max_devices": max_dev,
...
print("=== ACCOUNT-BASED LICENSING ENDPOINTS ===")
print("Copy the code above into server_app.py")
print("\\nEndpoints added:")
...
print("- GET /products (updated with color_hex)")
