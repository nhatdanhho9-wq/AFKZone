...
def get_products(active_only: bool = True, db: Session = Depends(get_db)):
    ...
    for r in results:
        ...
        products.append({
            ...
            "display_order": r[7],
            "description": r[8],
            "color_hex": r[9]  # NEW: from tiers table
        })
...
# ==================== USER AUTHENTICATION (Account-Based Licensing) ====================
...
def get_user_licenses(user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    for lic in licenses:
        ...
        result.append({
            "license_key": lic[0], "tier": lic[1], "duration_days": lic[2],
...
def get_user_activation_history(device_id: str, user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    ...
    for h in history:
        ...
        result.append({
            "license_key": h[0], "tier": h[1], "expires_at": expires.isoformat() if expires else None,
...
def update_device_alias(device_id: str, req: AliasRequest, user: dict = Depends(verify_user_token), db: Session = Depends(get_db)):
    """Update device alias (friendly name)"""
    device = db.execute(text("""
        SELECT ld.license_key FROM license_devices ld JOIN licenses l ON ld.license_key = l.license_key
        WHERE ld.device_id = :device_id AND l.user_id = :user_id
    """), {"device_id": device_id, "user_id": user["user_id"]}).fetchone()
    ...
    db.execute(text("UPDATE license_devices SET alias = :alias WHERE device_id = :device_id"), 
               {"alias": req.alias, "device_id": device_id})
    db.commit()
